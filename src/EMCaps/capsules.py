...
class ConvCaps(nn.Module):
    ...
    """
    def __init__(self, device, B=32, C=32, K=3, P=4, stride=2, iters=3,
        ...
        self._lambda = 1e-03
        self.device = device
        self.ln_2pi = torch.FloatTensor(1).fill_(math.log(2*math.pi)).to(device)
        # params
    ...
    def caps_em_routing(self, v, a_in, C, eps):
        ...
        r = torch.FloatTensor(b, B, C).fill_(1./C).to(self.device)
    ...
    def add_coord(self, v, b, h, w, B, C, psize):
        ...
        coor_h = torch.FloatTensor(1, h, 1, 1, 1, self.psize).fill_(0.).to(self.device)
        coor_w = torch.FloatTensor(1, 1, w, 1, 1, self.psize).fill_(0.).to(self.device)
